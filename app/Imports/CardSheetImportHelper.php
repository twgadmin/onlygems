...
class CardSheetImportHelper implements ToCollection, WithStartRow, ToModel, WithBatchInserts
// , ShouldQueue
{
    public function collection(Collection $rows)
    {        
        foreach($rows as $key=>$row){
         echo $key.' ---- '.$row[0].' ---- '.$row[3].'<br>';

            $updated = Card::where('internal_serial_number', $row[3])
            ->update([
            'cardhedger_internal_serial_number' => $row[0]
            ]);
